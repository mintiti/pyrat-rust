...
/// `first_moves` contains every direction that starts an optimal path.
...
/// Shortest path(s) between two cells. Returns None if unreachable.
/// `first_moves` lists every direction that starts an optimal path.
pub fn shortest_paths(
    ...
    dist[from.to_index(width)] = 0;
    heap.push(Reverse((0, from)));

    while let Some(Reverse((d, u))) = heap.pop() {
        let u_idx = u.to_index(width);
...
pub fn distances_from(
    ...
    while let Some(Reverse((d, u))) = heap.pop() {
        ...
        for (neighbor, w) in graph::neighbors(u, move_table, mud) {
            let new_dist = d + w as u32;
            let n_idx = neighbor.to_index(width);
    ...
    for (neighbor, w) in graph::neighbors(u, move_table, mud) {
        ...
        if new_dist < dist[n_idx] {
            ...
            first_moves[n_idx] = if u == source {
                vec![graph::direction_between(source, neighbor).unwrap()]
            } else {
...
mod tests {
    ...
    fn same_position() {
        ...
        let result = shortest_paths(c(1, 1), c(1, 1), w, h, &mt, &mud).unwrap();
    ...
    fn adjacent_open() {
        let (mt, mud, w, h) = open_grid(3, 3);
        let result = shortest_paths(c(0, 0), c(1, 0), w, h, &mt, &mud).unwrap();
    ...
    fn open_grid_manhattan() {
        ...
        let result = shortest_paths(c(0, 0), c(4, 4), w, h, &mt, &mud).unwrap();
    ...
    fn wall_forces_detour() {
        ...
        let result = shortest_paths(c(1, 0), c(1, 1), w, h, &mt, &mud).unwrap();
    ...
    fn mud_adds_cost() {
        ...
        let result = shortest_paths(c(0, 0), c(0, 1), w, h, &mt, &mud).unwrap();
        assert_eq!(result.cost, 3);
    }
    ...
    fn unreachable_returns_none() {
        ...
        let result = shortest_paths(c(0, 0), c(2, 2), w, h, &mt, &mud);
    ...
    fn tie_breaking_multiple_first_moves() {
        ...
        let (mt, mud, w, h) = open_grid(3, 3);
        let result = shortest_paths(c(0, 0), c(1, 1), w, h, &mt, &mud).unwrap();
    ...
    fn forced_through_mud_exact_cost() {
        ...
        let result = shortest_paths(c(0, 0), c(4, 0), w, h, &mt, &mud).unwrap();
    ...
    fn nearest_cheese_simple() {
        ...
        let results = nearest_cheeses(c(0, 0), &cheese, w, h, &mt, &mud);
    ...
    fn nearest_cheese_equidistant() {
        ...
        let results = nearest_cheeses(c(2, 2), &cheese, w, h, &mt, &mud);
    ...
    fn nearest_cheese_empty() {
        let (mt, mud, w, h) = open_grid(3, 3);
        let results = nearest_cheeses(c(0, 0), &[], w, h, &mt, &mud);
    ...
    fn nearest_cheese_on_position() {
        ...
        let results = nearest_cheeses(c(1, 1), &cheese, w, h, &mt, &mud);
        assert_eq!(results.len(), 1);
    ...
    fn distances_open_grid_manhattan() {
        let (mt, mud, w, h) = open_grid(3, 3);
        let dists = distances_from(c(0, 0), w, h, &mt, &mud);
